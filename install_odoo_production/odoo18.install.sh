...
############## PostgreSQL tuning (Odoo co-hosted, sized from RAM/cores) #######
# Applies memory/parallelism settings computed from this server via ALTER
# SYSTEM (revertible). Conservative because Odoo runs on the same box.
bash $DIR_PATH/pg_tune.sh --apply
##############################################################################
...
############# Auto worker + memory tuning (from this server's RAM/cores) ######
# Appends workers / limit_memory_hard / limit_memory_soft / limit_request /
# max_cron_threads computed from RAM and cores. limit_time_* is filtered out so
# the higher values set in the config above are kept.
# -m 60: Odoo may budget 60% of RAM, leaving room for the co-hosted, tuned
# PostgreSQL (shared_buffers ~20%) plus the OS and filesystem cache.
bash $DIR_PATH/workers_analyzer.sh -m 60 | grep -vE 'limit_time' | sudo tee --append $PATHBASE/config/odoo$VCODE.conf
##############################################################################
...
# pgtune / workers helper commands (re-run to re-tune if the hardware changes)
sudo cp $DIR_PATH/pg_tune.sh $PATHBASE/scripts/pgtune
sudo cp $DIR_PATH/workers_analyzer.sh $PATHBASE/scripts/workers
sudo chmod +x $PATHBASE/scripts/pgtune $PATHBASE/scripts/workers
